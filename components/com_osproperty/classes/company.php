...
class OspropertyCompany{
	...
	static function display($option,$task){
		...
		switch ($task){
			...
			case "company_cancelagent":
				global $mainframe;
				$mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_agent&Itemid='.JRequest::getInt('Itemid',0)));
			break;
		}
	}
	...
	static function saveCompanyInfo($option){
		...
		$db = JFactory::getDbo();
		$user = JFactory::getUser();
		$country = JRequest::getVar('country',HelperOspropertyCommon::getDefaultCountry());
		//check to see if user uploaded new state
		$nstate = JRequest::getVar('nstate','');
		if($nstate != ""){
		...
		if ( !empty($_FILES['file_photo']['name']) && $_FILES['file_photo']['error'] == 0 &&  $_FILES['file_photo']['size'] > 0 ) {
			$imagename = OSPHelper::processImageName(time()."_".$_FILES['file_photo']['name']);
			if(!HelperOspropertyCommon::checkIsPhotoFileUploaded('file_photo')){
				$needs = array();
				$needs[] = "ccompanydetails";
				$needs[] = "company_edit";
				$itemid  = OSPRoute::getItemid($needs);
				$mainframe->redirect(JRoute::_("index.php?option=com_osproperty&task=company_edit&Itemid=".$itemid),JText::_('OS_ALLOW_FILE').": *.jpg, *.jpeg");
			}elseif (move_uploaded_file($_FILES['file_photo']['tmp_name'],PATH_STORE_PHOTO_COMPANY_FULL.DS.$imagename)){
				...
				if($nwidth < $owidth){ //only resize when the image width is smaller
					$nheight = round((100*$oheight)/$owidth);
		...
		$needs[] = "company_edit";
		$itemid  = OSPRoute::getItemid($needs);
		$mainframe->redirect(JRoute::_("index.php?option=com_osproperty&task=company_edit&Itemid=".$itemid),$msg);
	}
	...
	static function editCompany($option){
		...
		$lists['country'] = HelperOspropertyCommon::makeCountryList($row->country,'country','onchange="change_country_company(this.value,'.$row->state.','.$row->city.')"',JText::_('OS_SELECT_COUNTRY'),'style="width:150px;"');

		if(OSPHelper::userOneState()){
			$lists['state'] = "<input type='hidden' name='state' id='state' value='".$row->state."'/>";
		}else{
			$lists['state'] = HelperOspropertyCommon::makeStateListAddProperty($row->country,$row->state,'state','onchange="change_state_company(this.value,'.intval($row->city).')" class="input-small"',JText::_('OS_SELECT_STATE'),'');
		}
	...
	static function editAgent($option){
		...
		$sortby = JRequest::getVar('sortby','a.name');
	...
	static function listingProperties($option,$id){
		...
		$limit = $mainframe->getUserStateFromRequest('com_osproperty.company.limit', 'limit', 20, 'int');
	...
	static function companyListing($option){
		global $mainframe,$configClass,$lang_suffix;
		$document = JFactory::getDocument();
		OSPHelper::generateHeading(1,$configClass['general_bussiness_name']." - ".JText::_('List companies'));
		$db = JFactory::getDBO();
		$limit = JRequest::getInt('limit',20);
		$limitstart = JRequest::getInt('limitstart',0);
		$orderby = JRequest::getVar('orderby','a.company_name');
		...
		if(count($rows) > 0){
			for($i=0;$i<count($rows);$i++){
				$agentlist = "";
				$row = $rows[$i];
				$db->setQuery("Select a.id from #__osrs_agents as a inner join #__users as b on b.id = a.user_id where a.company_id = '$row->id'");
				//echo $db->getQuery();
				$agents = $db->loadObjectList();
				if(count($agents) > 0){
					$agentlist = "";
	...
	static function companyDetails($option,$id){
		...
		$db = JFactory::getDbo();
		if(intval($id) == 0){
			JError::raiseError( 404, JText::_('OS_COMPANY_NOT_AVAILABLE') );
		}
		$query = "Select a.*,b.country_name,c.state_name$lang_suffix as state_name from #__osrs_companies as a"
				." inner join #__osrs_countries as b on b.id = a.country"
				." left join #__osrs_states as c on c.id = a.state"
				." where a.id = '$id'";
		$db->setQuery($query);
		$company = $db->loadObject();
		...
		if($company->published == 0){
			...
			//$itemid = OSPRoute::getItemid($needs);
			//$mainframe->redirect(JRoute::_('index.php?option=com_osproperty&view=lcompanies&Itemid='.$itemid),JText::_('OS_COMPANY_NOT_AVAILABLE'));
			JError::raiseError( 404, JText::_('OS_COMPANY_NOT_AVAILABLE') );
		}
	...
	static function submitContact($option,$id){
		...
		if($comment_security_code == ''){
			...
			$mainframe->redirect(JRoute::_("index.php?option=com_osproperty&task=company_info&id=".$id."&Itemid=".JRequest::getInt('Itemid',0)),$msg);
		...
		$comment_title  = JRequest::getVar('comment_title','');
		...
		$mainframe->redirect(JRoute::_("index.php?option=com_osproperty&task=company_info&id=".$id."&Itemid=".JRequest::getInt('Itemid',0)),JText::_('OS_EMAIL_HAS_BEEN_SENT'));
	}
	...
	static function agentStatus($option,$cid,$state){
		...
		$mainframe->redirect(JRoute::_("index.php?option=com_osproperty&task=company_agent"),$msg);
	}
	...
	static function processToAssignAgentToCompany($option){
		...
		$mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_addnew'),JText::_('OS_AGENT_HAS_BEEN_ASSIGNED_TO_YOUR_COMPANY'));
	}
	...
	static function modifyAgent($option,$id){
		...
		$lists['city'] = HelperOspropertyCommon::loadCityAddProperty($option,$default_state,$agent->city);
		
		$optionArr = array();
		$optionArr[] = JHTML::_('select.option',1,JText::_('OS_YES'));
		$optionArr[] = JHTML::_('select.option',0,JText::_('OS_NO'));
		$lists['published']   = JHTML::_('select.genericlist',$optionArr,'published','class="input-mini"','value','text',$agent->published);
		
		$optionArr = array();
		$optionArr[] = JHTML::_('select.option',1,JText::_('OS_YES'));
		$optionArr[] = JHTML::_('select.option',0,JText::_('OS_NO'));
		$lists['featured']   = JHTML::_('select.genericlist',$optionArr,'featured','class="input-mini"','value','text',$agent->featured);
	...
	static function saveAgent($option,$id,$save){
		...
		if($id == 0){
			...
			if($countuser > 0){
				$mainframe->redirect("index.php?option=com_osproperty&task=company_addagents",JText::_('OS_USER_IS_ALREADY_EXISTS'));
			}
			...
			if($countemail > 0){
				$mainframe->redirect("index.php?option=com_osproperty&task=company_addagents",JText::_('OS_EMAIL_IS_ALREADY_EXISTS'));
			}
			...
			$data['password2']	= JRequest::getVar('password1','');
			...
			// Bind the data.
			if (!$user->bind($data)) {
				$mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_addagents'),JText::sprintf('COM_USERS_REGISTRATION_BIND_FAILED', $user->getError()));
				return false;
			}		
			// Store the data.
			if (!$user->save()) {
				$mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_addagents'),JText::sprintf('COM_USERS_REGISTRATION_SAVE_FAILED', $user->getError()));
				return false;
			}
		}
		...
		if($save == 1){
			...
			$mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_editagent&id='.$id),$msg);
		}
	}
	...
	static function manageProperties($option){
		...
		$filter_search = JRequest::getVar('filter_search','');
		$filter_search = $db->escape($filter_search);
		$sortby = JRequest::getVar('sortby','a.id');
		$orderby = JRequest::getVar('orderby','desc');
	...
	static function companyRegister($option){
		...
		$lists['country'] = HelperOspropertyCommon::makeCountryList('','country','onchange="change_country_company(this.value,\''.$row->state.'\',\''.$row->city.'\')"',JText::_('OS_SELECT_COUNTRY'),'');
		
		//$lists['state'] = HelperOspropertyCommon::makeStateListAddProperty('','','state','onchange="loadCity(this.value,\''.$row->city.'\')"',JText::_('OS_SELECT_STATE'),'');
		//$lists['city'] = HelperOspropertyCommon::loadCityAddProperty($option,0,0);
		...
		$lists['city'] = HelperOspropertyCommon::loadCityAddProperty($option,$default_state,0);
	...
	static function saveNewCompany($option){
		...
		if($configClass['captcha_agent_register'] == 2){
			...
			if(!$res[0]){
			    $mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_register&Itemid='.JRequest::getInt('Itemid',0)),JText::_('OS_SECURITY_CODE_IS_WRONG'));
			}
		}
		...
		if(intval($user->id) == 0){
			...
			$countuser = $db->loadResult();
			if($countuser > 0){
				$mainframe->redirect(JRoute::_("index.php?option=com_osproperty&task=company_register&Itemid=".$itemid),JText::_('OS_USER_IS_ALREADY_EXISTS'));
			}
			$email = OSPHelper::getStringRequest('email','','post');
			$db->setQuery("Select count(id) from #__users where email like '$email'");
			$countemail = $db->loadResult();
			if($countemail > 0){
				$mainframe->redirect(JRoute::_("index.php?option=com_osproperty&task=company_register&Itemid=".$itemid),JText::_('OS_EMAIL_IS_ALREADY_EXISTS'));
			}
			//register new user in the case user is not registered-user
			// Get the form data.
			//$data	= JRequest::getVar('user', array(), 'post', 'array');
			...
			// Bind the data.
			if (!$user->bind($data)) {
				$mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_register&Itemid='.$itemid),JText::sprintf('COM_USERS_REGISTRATION_BIND_FAILED', $user->getError()));
				return false;
			}	
			
			// Store the data.
			if (!$user->save()) {
				$mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_register&Itemid='.$itemid),JText::sprintf('COM_USERS_REGISTRATION_SAVE_FAILED', $user->getError()));
				return false;
			}
		...
		if(is_uploaded_file($_FILES['image']['tmp_name'])){
			...
			if(!HelperOspropertyCommon::checkIsPhotoFileUploaded('image')){
				...
				$mainframe->redirect(JRoute::_('index.php?option=com_osproperty&task=company_register&Itemid='.$itemid),$msg);
			}else{ //upload file
				$imagename = OSPHelper::processImageName(time()."".str_replace(" ","",$_FILES['image']['name']));
				if (move_uploaded_file($_FILES['image']['tmp_name'],PATH_STORE_PHOTO_COMPANY_FULL.DS.$imagename)){
					// copy image before resize
					copy(PATH_STORE_PHOTO_COMPANY_FULL.DS.$imagename,PATH_STORE_PHOTO_COMPANY_THUMB.DS.$imagename);
					// resize image just copy and replace it selft
					require_once (JPATH_COMPONENT_ADMINISTRATOR.DS.'helpers'.DS.'image.php');
		...
		if($configClass['auto_approval_company_register_request'] == 1){
			if(($useractivation == 0) or ($already_registered == 1)){
				$needs = array();
				$needs[] = "ccompanydetails";
				$needs[] = "company_edit";
				$itemid = OSPRoute::getItemid($itemid);
	...
	function myPlans(){
		...
		if($configClass['integrate_membership'] == 1){
			...
			HTML_OspropertyCompany::listCompanyPlans($plans);
		}else{
			JError::raiseError( 500, JText::_('OS_YOU_HAVE_NOT_GOT_PERMISSION_GO_TO_THIS_AREA') );
		}
	}
