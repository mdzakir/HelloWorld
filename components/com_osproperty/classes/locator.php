...
class OspropertyLocator{
	...
	static function locatorSearch($option){
		...
		$max_locator_results = isset($configs['max_locator_results'])? $configs['max_locator_results']:'100';
		...
		$locator_type   = JRequest::getVar('locator_type',$locator_type_ids);
		$locator_search	= JRequest::getInt('locator_search',0);
		$location		= OSPHelper::getStringRequest('location',$configClass['default_location'],'');
		$lists['location'] = $location;
		$default_search	= JRequest::getInt('default_search',0);
		$keyword		= OSPHelper::getStringRequest('keyword','','');
		$keyword		= $db->escape($keyword);
		//$category_id 	= JRequest::getInt('category_id',0);
		$categoryArr	= JRequest::getVar('categoryArr');
		...
		$agent_id    	= JRequest::getInt('agent_id','');
		$radius_search  = JRequest::getInt('radius_search',isset($configs['default_radius'])? $configs['default_radius']:'20');
		...
		$nbath			= OSPHelper::getStringRequest('nbath',0,'');
		$nbed			= OSPHelper::getStringRequest('nbed',0,'');
		$nroom			= OSPHelper::getStringRequest('nroom',0,'');
		...
		if (($location != '') and ($radius_search > 0) and ($status == "OK")){
        ...
        $lists['sort'] = Jhtml::_('select.genericlist',$optionArr,'sortby','class="input-medium"','value','text',$lists['sortby']);
        ...
        $lists['order'] = Jhtml::_('select.genericlist',$optionArr,'orderby','class="input-medium"','value','text',$lists['orderby']);
		// creat option search
		// keyword
		...
		$lists['type'] = JHTML::_('select.genericlist',$typeArr,'property_type','class="input-large"','value','text',$property_type);
		...
		$lists['radius'] = JHtml::_('select.genericlist',$radiusArr,'radius_search','class="input-medium  search-query" style="margin-bottom:2px !important"', 'value', 'text',$radius_search);
		...
		if(intval($user->id) > 0){
			...
			if($special){
				$where .= " and `access` in (0,1,2) ";
			}else{
				$where .= " and `access` in (0,1) ";
			}
		}else{
			$where .= " and `access` = '0' ";
		}
		...
		if (($location != '') and ($radius_search > 0)){
			//echo $status;
			if ($status == "OK") {
		...
		$group_by = "";
		if (($dosearch == 1) and ($location != "")){
			$db->setQuery($select.' '.$from.' '.$where.' '.$group_by.' '.$Order_by);
			//echo $db->getQuery();
		...
		HTML_OspropertyLocator::locatorSearchHtml($option,$rows, $configs,$lists,$locator_type,$search_lat,$search_long);
	}
