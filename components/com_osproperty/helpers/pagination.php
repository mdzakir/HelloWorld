<?php
/*------------------------------------------------------------------------
# pagination.php - Ossolution Property
# ------------------------------------------------------------------------
...
if (!class_exists('OSPJPagination')){
	class OSPJPagination extends JObject
	{
		/**
		...
		function _list_render($list)
		{
			...
			if($lang->isRTL())
			{
				$html .=  "<div style='margin:0;padding:0;float:right;'>".$list['previous']['data']."</div>";
				$html .= "<div style='margin:0;padding:0;float:right;'>".$list['start']['data']."</div>";
				$list['pages'] = array_reverse( $list['pages'] );
				foreach( $list['pages'] as $page ) {
					$html .= "<div class='hidden-phone' style='margin:0;padding:0;float:right;'>".$page['data']."</div>";
				}
				$html .= "<div style='margin:0;padding:0;float:right;'>".$list['end']['data']."</div>";
				$html .= "<div style='margin:0;padding:0;float:right;'>".$list['next']['data']."</div>";
			}
			else
			{
			//	$html .= '&lt;&lt; ';
				$html .= "<div style='margin:0;padding:0;float:left;'>".$list['start']['data']."</div>";
			//	$html .= ' &lt; ';
				$html .= "<div style='margin:0;padding:0;float:left;'>".$list['previous']['data']."</div>";
				foreach( $list['pages'] as $page ) {
					$html .= "<div class='hidden-phone' style='margin:0;padding:0;float:left;'>".$page['data']."</div>";
				}
				$html .= "<div style='margin:0;padding:0;float:left;'>". $list['next']['data']."</div>";
			//	$html .= ' &gt;';
				$html .= "<div style='margin:0;padding:0;float:left;'>". $list['end']['data']."</div>";
				//$html .= ' &gt;&gt;';
			}
			$html .= "</ul>";
			return $html;
		...
		function _item_active(&$item)
		{
			global $mainframe;
				...
				if($item->base>0)
				{
					return "<li style='margin:0 !important;padding:0 !important;'><strong><a href=\"javascript: document.ftForm.limitstart.value=".$item->base."; document.ftForm.submit();\" onclick=\"javascript: document.ftForm.limitstart.value=".$item->base."; document.ftForm.submit();\" >".$item->text."</a></strong></li>";
				}
				else
				{
					return "<li style='margin:0 !important;padding:0 !important;'><strong><a href=\"javascript: document.ftForm.limitstart.value=".$item->base."; document.ftForm.submit();\" title=\"".$item->text."\" onclick=\"javascript: document.ftForm.limitstart.value=0; document.ftForm.submit();\" >".$item->text."</a></strong></li>";
				}
		}
		...
		function _item_inactive(&$item)
		{
			...
			if ($mainframe->isAdmin()) {
				...
				return "<li style='margin:0 !important;padding:0 !important;'><span class=\"pagenav\">" . $item->text . "</span></li>";
			}
		}
		...
		function _buildDataObject()
		{
			...
			$data->all	= new OSPJPaginationObject(JText::_('OS_VIEW_ALL'));
			if (!$this->_viewall) {
				$data->all->base	= '0';
			...
			$data->start	= new OSPJPaginationObject(JText::_("OS_START"));
			$data->previous	= new OSPJPaginationObject(JText::_("OS_PREVIOUS"));
			...
			$data->next	= new OSPJPaginationObject(JText::_("OS_NEXT"));
			$data->end	= new OSPJPaginationObject(JText::_("OS_END"));
			...
			for ($i = $this->get('pages.start'); $i <= $stop; $i ++)
			{
				...
				$data->pages[$i] = new OSPJPaginationObject($i);
	...
	if (!class_exists('JPaginationClone')){
		class JPaginationClone extends JObject
		{
			...
			function _buildDataObject()
			{
				...
				$data->all	= new OSPOSPJPaginationObject(JText::_('View All'));
				if (!$this->_viewall) {
					$data->all->base	= '0';
					$data->all->link	= JRoute::_("&limitstart=");
				}
				...
				// Set the start and previous data objects
				$data->start	= new OSPJPaginationObject(JText::_("Start"));
				$data->previous	= new OSPJPaginationObject(JText::_("Previous"));
				...
				// Set the next and end data objects
				$data->next	= new OSPJPaginationObject(JText::_("Next"));
				$data->end	= new OSPJPaginationObject(JText::_("End"));
	...
	if (!class_exists('OSPJPaginationObject')){
		class OSPJPaginationObject extends JObject
		{
			var $text;
			var $base;
			var $link;
