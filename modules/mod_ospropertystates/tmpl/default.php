...
<div class="row-fluid">
	...
	if(count($states) > 0){
		...
		for($i=0;$i<count($states);$i++){
			...
			$j++;
			if($list_type == 0){
				$link = JRoute::_("index.php?option=com_osproperty&task=property_listing&state_id=".$state->id."&Itemid=".$itemid);
			}else{
				$link = JRoute::_("index.php?option=com_osproperty&task=property_listing&city=".$state->id."&Itemid=".$itemid);
			}
			?>
